...
<div class="container mt-5">
    <h1 class="text-center mb-4"><?php echo $post_id ? 'Редактировать пост' : 'Создать новый пост'; ?></h1>

    <form id="edit-post-form" method="POST">
        <div class="form-group">
            <label for="title">Заголовок:</label>
            <input type="text" name="title" id="title" class="form-control" required value="<?php echo $post_id ? $post['title'] : ''; ?>">
        </div>
        <div class="form-group">
            ...
            <div class="bb-editor-buttons">
                ...
                <button type="button" class="bb-editor-button" onclick="insertTag('underline')">Подчеркнутый</button><br>
                  <button type="button" class="bb-editor-button" onclick="removeFormat()">Удалить форматирования</button><br>
                ...
                <button type="button" class="bb-editor-button" style="color: green;" onclick("setColor('black')">Черный</button>
            ...
            <div class="bb-editor-content" id="content" contenteditable="true" required><?php echo $post_id ? $post['content'] : ''; ?></div>
        </div>
        <input type="hidden" id="post_id" name="post_id" value="<?php echo $post_id; ?>">
        <input type="submit" class="btn btn-primary w-100" value="<?php echo $post_id ? 'Редактировать пост' : 'Создать пост'; ?>">
    </form>
</div>
...
$(document).ready(function() {
  $('#edit-post-form').submit(function(e) {
      ...
      $.ajax({
          type: 'POST',
          url: 'includes/engine.php?action=edit_post',
          data: {
              post_id: post_id,
              title: title,
              content: content,
